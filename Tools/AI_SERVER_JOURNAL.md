...
- Release evidence: the production gate completed with `ok=true`; the complete LuaM integration suite passed 531/1/0 on this commit before an explicitly operator-authorized test-override, and the post-deploy critical-feature smoke passed 92/92. Source package SHA256 is `6301ef1ad57199d410bb00fb93f64a9c222a9e6bcfa5e9e1893097eb0164fc85`, server package SHA256 is `a39bd79b5a066d931050a9b299d931e925a5f88dfd5155a984aa5dc6ec69b3a5`, client package SHA256 is `7c6b25b450a9f3f2c4d8dad944ac148ec629631bd18cf359e673a43998df0b95`, and binary receipt SHA256 is `8beed6e855f255f00319748fa56573e215363a757a52a88f96d0753cb7050750`.
- Client: the 376,894,983-byte client was published to the static host and returns HTTP 200. `/info` advertises build `7c6b25b450a9f3f2c4d8dad944ac148ec629631bd18cf359e673a43998df0b95` and the matching download URL.
- Deployment safety: the authenticated ship-save barrier completed with attempted/saved/failed/active-remaining `0/0/0/0` and froze lifecycle mutations. Required backups are `/opt/monolith-ds/backups/server-luam-20260818-060320`, `/opt/monolith-ds/backups/server_config-before-luam-20260818-060320.toml` (SHA256 `39cf4aa9b83a8adf17092467e64a2b627f818167d09c27401b6e30854543556a`, live config unchanged), and `/opt/monolith-ds/backups/data-luam-20260818-060320.tar.gz`.
- Post-deploy health: `monolith-ds.service` is active/running with `NRestarts=0` and `ExecMainStatus=0`; round 203 is in lobby with zero players. The new `/info` metadata is served. The stale `Pathfinder` active lease from the pre-release round is reconciled by the normal startup lease recovery path.
- Closure: release policy is frozen again, authorization fields are cleared, and the batch is `local-package-only`. No external AI-gateway mutation occurred.
...
- Sound constraint: the deployed local bridge `announce` action uses the silent server-announcement channel. A real announcement sound is available only from an authenticated connected in-game admin command/UI; there is no documented safe host/bridge audio action, live AI broadcast TTS is disabled, and announcement-only authority was not expanded into a configuration change or restart. The corrected visible announcement therefore contains no textual siren, but no separate audio cue was emitted by this bridge operation.
- Preflight: repository and installed server journals matched SHA256 `867fd65791e95f5d7a83ae230a030e017a9c4e71f5ada14d2a7f4f2ef7fd3ce5`; `monolith-ds.service` was active with `NRestarts=0`/`ExecMainStatus=0`; `/status` reported running round 192 with four players. The bridge inbox and outbox existed as `monolith:monolith` mode `0644`, and both required local-bridge flags were true. The AI gateway was healthy, but `tts_enabled=false`.
- Delivery/verification: the first unprivileged append was rejected before mutation and left inbox/outbox at 5,516/6,973 bytes. The owner-scoped append delivered the first line at `2026-07-31T18:25:55.4572300Z`, growing them to 5,728/7,244 bytes. The first corrected payload decode failed before mutation and left both sizes unchanged; the valid owner-scoped append delivered the corrected line at `2026-07-31T18:27:02.8378823Z`, growing them to 5,924/7,499 bytes. Each delivered line has exactly one `bridge announcement sent` outbox row and one matching service-log confirmation; no ambiguous retry or additional bridge command occurred.
- Post-state/recovery: service remained active with zero restarts/status errors, and `/status` still reported round 192 with four players. No code, package, client, configuration, database, ship, firewall, gateway, service, or round mutation occurred. Announcements are transient and require no rollback.

## 2026-07-31T15:23:28Z -- nanonite space-moth warning announced in game

- Scope/result: sent one operator-requested player-facing server announcement during round 192: `ВНИМАНИЕ! Нанонанитная космомоль вошла в сектор. Рой способен разъедать броню и просаживать энергетические щиты. Избегайте контакта, держите дистанцию и контролируйте состояние защиты.` No code, package, client, configuration, database, ship, firewall, gateway, service, or round mutation occurred.
- Preflight: `monolith-ds.service` was active; `/status` reported round 192 running with four players. Repository and installed server journals matched SHA256 `8cb67dc2ab6192b51e027877aa9692737972188839eacad3cfaebcf3414075ec`. The existing local bridge inbox/outbox were `monolith:monolith` mode `0644`, and the live config reported both `local_bridge_enabled=true` and `local_bridge_unsafe_actions_enabled=true` without exposing any other configuration value.
- Delivery/verification: appended exactly one UTF-8 `announce|...` command to `/opt/monolith-ds/data/luam/ai_inbox.txt` through the intended local bridge interface. Within two seconds, the outbox grew by 411 bytes and recorded `bridge announcement sent`; no retry or duplicate command was issued. Post-send status remained service active, round 192, four players.
- Recovery/next action: the announcement is transient and needs no rollback. If the warning must be repeated later, issue a new explicit announcement rather than replaying or editing the current inbox line.

## 2026-07-31T12:56:05Z -- server-only hostile cultist hotfix deployed and verified

- Scope/result: deployed the operator-authorized server-only hostile-mob repair without publishing or changing the client archive. Atmosphere-resistant hostile NPCs no longer abandon combat because vacuum is misclassified as a personal emergency. Blood cultists now evaluate every 0.25 seconds, perceive targets at 18 tiles, tolerate threat groups up to 12, gain +6 Slash melee damage, and apply a one-time 1.25x multiplier to their projectile and hitscan damage.
- Exact source/release: the hotfix was implemented from the exact live base `b3dc7ff09f08fbc1e98d56dc26a4b63d7f125f6c` in clean commit `a7f4e8aa640fe05de62d1cfed2c2f6feff6c5e3c`. The complete production gate passed both policy test filters, both required smokes, independent source verification, binary build, surface audit with zero violations, and client/server dry-runs. Source package SHA256 is `4b190e1cb045c767dea04bc4f8eb3d270b004f54303d89681b804e0a405f4af1`, payload digest `83f1c6319b0126a5d054a7f724341560f20bec1a85ee488bdc4c67aa82931516`, worktree digest `c883d87e64ced3b2ce11eb3a85ce4e4c47dafd67612904c0f774c415385a449e`, server archive SHA256 `dc5c596ef850b433700db960344f1284b39c48baed0c631c4f8ba66a2f9dcf94`, and release receipt SHA256 `762e611fc004e295fdc846b238838316d1dd74aa59ffef310608c36f1f417755`.
- Client preservation: the builder reused the exact existing 340,219,496-byte client archive SHA256/build `a858829b1d9d029c00b05eabe39d9c98e1c98075b91be783db6525e663acbbc7`; no client publication occurred. Post-deploy `/info` advertises that same hash and URL, whose HEAD request returns HTTP 200 with the exact byte length. The main-worktree ServerNews entry `2026073101` remains queued for the next global client update rather than being falsely advertised by this server-only rollout.
- Safety/deployment: fresh preflight found round 190 running with two players, 11,614,441,472 bytes free, service active with `NRestarts=0`/`ExecMainStatus=0`, config SHA256 `d7cdac9f9df5049a785ffeb6730516a33d7fe6dc806e7fe4b50042b933251940`, and query-only SQLite `quick_check=ok` with one active/restoring ship and one lease. Under the operator's explicit restart-with-players authorization, a temporary `inet luam_deploy_guard` blocked only non-loopback UDP/1212 and the unchanged old service was restarted into a zero-session lobby. Authenticated ship-save barrier `57e52835-c17c-4f66-87c1-ea2dcc1b9040`, created `2026-07-31T12:52:48.853949Z`, froze publication and reported attempted/saved/failed/activeRemaining `0/0/0/0`. The official receipt-bound deploy then stopped the service, created required backups, atomically swapped the server, started round 192, and passed status/info verification as tag `20260731-hostile-mob-server-hotfix`. The guard was deleted in the local `finally` path and proved absent; two UDP/1212 listeners are present.
- Recovery: server rollback is `/opt/monolith-ds/backups/server-20260731-hostile-mob-server-hotfix` and contains the prior DLL SHA256 `9ba12761f9285f44d69b786da5d6be4664dc7cf4e9365932518a58bf6f000c46`. Config rollback is `/opt/monolith-ds/backups/server_config-before-20260731-hostile-mob-server-hotfix.toml`, SHA256 `d7cdac9f9df5049a785ffeb6730516a33d7fe6dc806e7fe4b50042b933251940`, `root:root` mode `0600`. Data rollback is `/opt/monolith-ds/backups/data-20260731-hostile-mob-server-hotfix.tar.gz`, 385,419,615 bytes, SHA256 `e40502fb3fb5fff9c91f55e50d585ceb6a5e7cd7c3927e059785010c975b4fb2`, `root:root` mode `0600`, and passes full `gzip -t`.
- Post-deploy health: `monolith-ds.service` is active with `NRestarts=0` and `ExecMainStatus=0`; round 192 is in lobby with zero players. Installed `Content.Server.dll` SHA256 `eb8e3e335026bfea6637af633d4709573f4182bb243eb7e8de7036d7df20dd16` exactly matches the packaged DLL. Live config remained byte-identical, SQLite remains `quick_check=ok`, and the newly started server again has one active/restoring ship and one presence lease. The bounded post-start journal contains two readiness/advertisement markers and zero warning- or error-priority entries. Root has 10,958,090,240 bytes free (73% used); the uploaded deploy archive was removed.
- Gate notes/commands: the first attached gate exceeded its local one-hour shell limit while healthy child tests continued, then ended on a transient local-stack smoke; the isolated worktree's next complete tests/smokes passed but correctly refused packaging because ignored `server_config.remote.toml` was absent. Copying the existing local config byte-for-byte (SHA256 `5f6c54c53ee7216bfa40b7dbda27fd72fbc6177073bfe707c213ae68f326bd60`) made the seven-day round and feature validators pass; the final full production gate then passed. Neither failed local attempt touched production. Production commands were bounded preflight reads, the scoped guard/restart, the official receipt-bound deploy with empty `ConfigSourcePath`, required data backup, guard cleanup, and independent hash/archive/health verification. No private player or ship identity, payload, raw configuration, environment, or credential was read or recorded.
- Next action: perform an ordinary player smoke against cultists in vacuum and at range; if behavior is still passive, capture only bounded AI intent/error aggregates before considering another mutation. Keep the global ServerNews/prototype update queued for the next normal client release.

## 2026-07-31T10:07:43Z -- canonical journal reconciled; hostile server-only hotfix preflight

- Scope: read-only reconciliation and production preflight after the operator explicitly authorized restarting with connected players for the hostile/cultist hotfix. The only host mutation after finalizing this entry is the required journal-mirror installation; no package, client, service, configuration, database, ship, round, player, firewall, or gateway mutation occurred.
- Journal reconciliation: the installed `/opt/monolith-ds/AI_SERVER_JOURNAL.md` was newer than the repository copy: SHA256 `b78e118f45a05f17259dad2c238a216ce61a7ff3aa59378d28e2eb2027e3fa9c`, 272,282 bytes, 1,539 logical lines, `root:root` mode `0644`. Its complete copy was downloaded read-only to `C:\MonolithTemp\AI_SERVER_JOURNAL.host-20260731.md` and its newer 2026-07-30 entries were read. The authoritative live release is the receipt-bound Colossus gateway hotfix from clean commit `b3dc7ff09f08fbc1e98d56dc26a4b63d7f125f6c`: source SHA256 `7bedcdf5749efc0b3b55ead13aaccdf04fe74e3f2a7a09516f13ecb86d2eea3d`, server archive SHA256 `ba54ccc1cd547ca690b7913176a2a74aa33e8f40efa427933ab6026ec0f74c24`, client/build `a858829b1d9d029c00b05eabe39d9c98e1c98075b91be783db6525e663acbbc7`, receipt SHA256 `dee8efb53ae3a608a52e5486615b20a2c342f75f79ec5b1f52440f459da8eb48`, and installed `Content.Server.dll` SHA256 `9ba12761f9285f44d69b786da5d6be4664dc7cf4e9365932518a58bf6f000c46`.
- Reconciled intervening operations: the comprehensive connected update and Colossus gateway hotfix were deployed and verified; later narrative bridge announcements did not change gameplay; a populated SmartFridge save defect was diagnosed without mutation; one manually added map-only salvage component deleted a live shuttle whose prior durable snapshot returned safely to `Stored`; one stored-hull ownership transfer was committed and then exactly reversed after target clarification; the intended active vessel registry/live grid remained unchanged and a private no-restart deed-copy instruction was delivered. Private receipt contents and player/ship identities are not retained here.
- Current health: `monolith-ds.service` is active with `NRestarts=0` and `ExecMainStatus=0`; `/status` reports round 190 running on Colossus with three players and `/info` advertises exact build `a858829...`. Root storage is 42,174,005,248 bytes total with 11,622,653,952 bytes available (71% used). The current service process began at `2026-07-31 06:40:32 MSK`; a bounded lifecycle read shows a normal stop/start, `Ready`, and hub advertisement with the same installed DLL/build, but this restart was not described by the older installed journal. The two-hour hostile/save-failure aggregate was zero; the bounded error-priority count since startup was one and no deployment artifact changed.
- Fleet safety: query-only SQLite opened with `PRAGMA query_only=ON`; `quick_check=ok`. Aggregate ship states are 15 status-0 rows, one status-2 row, and 12 status-4 rows with one presence lease. The status-2/lease pair means a forced restart can discard live changes made after that ship's last durable snapshot; the official authenticated ship-save barrier and required data backup remain mandatory and must not be bypassed.
- Recovery baseline: current binary rollback is `/opt/monolith-ds/backups/server-luam-20260730-colossus-gateway-hotfix`; config rollback is `/opt/monolith-ds/backups/server_config-before-luam-20260730-colossus-gateway-hotfix.toml`; data rollback is `/opt/monolith-ds/backups/data-luam-20260730-colossus-gateway-hotfix.tar.gz` (SHA256 `fdb6ab66e2c4b2c2a79042ab311a2304901da2809a28455f23ccbdb3787841c6`, root-only mode `0600`, previously fully gzip-validated). A new deployment must create its own exact recovery set.
- Commands/outcomes: complete repository journal reads; read-only `scp` of the installed journal; SHA256/owner/mode/heading comparison; bounded `systemctl`, `/status`, `/info`, storage, DLL/backup, and lifecycle reads; query-only SQLite aggregate; recent error/save-failure counts; Git ancestry checks; and a SHA256-gated exact journal upload/install/owner/mode/service verification. Deployed commit `b3dc7ff...` exists locally and is 20 commits ahead of current `master`, so the hotfix must be built from that exact release anchor in an isolated worktree rather than from the dirty current branch.
- Next action: implement the hostile behavior and cultist tuning as a server-runtime-only change on clean commit `b3dc7ff...`, preserve the existing client build, run the complete receipt-bound server-only gate, then use the authorized guarded restart path only after the ship-save/data-backup barriers pass.
## 2026-07-30T10:45:49Z -- Colossus gateway hotfix diagnosed, validated, and authorized

- Scope/outcome: bounded read-only production diagnostics plus local implementation. The live Colossus gateway had three normally generated destination maps, but the 09:22Z automatic space cleanup removed their unprotected return endpoints while the generator continued counting the maps at capacity. This explains the reported UI briefly loading and then showing no assignments. No production file, service, client, config, database, ship, player, round, firewall, or gateway state was mutated in this iteration.
- Read-only commands: required repository journal/policy reads; installed journal SHA256/owner/mode comparison; `sudo -n systemctl show monolith-ds.service`; loopback `/status`; bounded `sudo -n journalctl -u monolith-ds.service` searches for gateway generation, cleanup, and failure records; bounded five-key gateway configuration inspection; `df -B1`; and `free -b`. One initial local query used the nonexistent `monolith.service` name before immediately correcting to `monolith-ds.service`. The latest combined SSH wrapper completed service/status/storage checks but its final `awk` memory formatter was expanded by PowerShell and failed; the corrected `free -b | sed -n '2p'` command succeeded.
- Current health/storage: installed journal matched repository SHA256 `7430cc04b3d6fbf239c34caddae38f5846c533169f35ccdbc5fdfe71383ed390`, owner `root:root`, mode `0644`. `monolith-ds.service` is active/running at PID 58105, `NRestarts=0`, `ExecMainStatus=0`, active since 2026-07-30 11:54:30 MSK. `/status` reports round 186 running on Colossus with one player. Root size/used/available are 42,174,005,248 / 26,210,881,536 / 13,799,051,264 bytes (66% used); available memory is 11,349,663,744 bytes.
- Local hotfix/evidence: generated gateway endpoints are cleanup-immune and self-heal on the same world through a one-second safety pass that refreshes open UIs. Procedural worlds use a validated 160-tile safety boundary from initialization, avoiding active physics-boundary replacement. Server/integration builds passed with zero errors; the final production-map/client-UI and existing rotation tests passed four independent repetitions, 8/8, and the related gateway/news set previously passed 3/3. A preliminary `prepare_luam_hotfix.ps1 -Scope Auto -Json` invocation hit the local 604-second wrapper timeout while the full LuaM integration filter was still running and is not accepted as a gate result. Its two exact worktree-owned test processes were verified by command line and stopped; no unrelated process was touched. The static feature-pack validator passed immediately afterward.
- Authorization/recovery: the operator authorized the matching update and restart. Policy permits only receipt-bound `client-static` and `server-release` for batch `20260730-colossus-gateway-hotfix` through 2026-07-30T16:00:00Z. No rollback is presently required because production is unchanged. The existing live rollback remains `/opt/monolith-ds/backups/server-luam-20260730-081938`; its associated data/config recovery facts remain in the preceding entry.
- Next action: install this canonical journal, commit the candidate, run `& .\Tools\ship_luam_release.ps1 -ConfigSourcePath ([string]::Empty) -Force -Json`, and continue to publication/swap only if every receipt-bound gate reports success.

## 2026-07-30T09:00:29Z -- comprehensive connected update deployed and verified

- Result/scope: completed the operator-authorized client/server rollout `luam-20260730-081938` from clean source commit `b5116fa12715cd0bf08d6177afbd16f2d183f877`. The live release connects the procedural gateway, shared radar/targeting, hardened Unknown conversations, Aibolit/rescue, round-local character behavior, persistent multi-ship storage and legacy call compatibility, late-join work, restricted regular-ghost scouting, selected safety fixes, and the viewport/HUD suspension repair. The external AI-gateway code/service was not deployed or restarted.
- Gate evidence: the final production gate completed with `ok=true`. Both required production test filters, `gateway-test`, `local-stack-smoke`, source verification, Linux server/client build, release-surface audit, and client/server dry-runs passed; surface violations were zero. Source archive SHA256 is `4779394f928a20ed9a6ee4ab5110a0f0145255e03af1b70b23e1facb52c690d0`, payload digest `97dba300c1d16db401f36c131c2e8ed062edee014246d0d829c2f61a50030e13`, worktree digest `82edc75013b7b39f14ce311c1b005b06f9e9f4741b6eaecfad5c3ecbe06fd37e`, policy SHA256 `0570e035d54a1ecc3d4dd72a5fe8fe5f5a480c9570bec4caf688f0353d33f081`, server archive SHA256 `47f7728cc1379822702e04482a1b776e781c9b61a608e3b92eea69bbd410bdd1`, client archive/build version SHA256 `7588a2cc53a508223cc35a2e3362919a3fbdc86fad6d09a8a2a769e8b9ff291e`, and binary receipt SHA256 `42a5342c564242e5a2dc0cf779e950f07b1904ccda20ed1db1cf0e2948948de3`. The deploy dry-run and live invocation used an empty `config_source_path`, so the complete production config was never replaced.
- Client publication: the receipt-bound 340,219,347-byte client was installed immutably at `http://188.127.225.57:1213/7588a2cc53a508223cc35a2e3362919a3fbdc86fad6d09a8a2a769e8b9ff291e/SS14.Client.zip`; installed SHA256 and size matched, nginx configuration validation passed, and both internal and external HEAD checks returned HTTP 200. `/info` advertises that exact version, hash, and URL with external-zip delivery.
- Gateway configuration: before restart, created root-only rollback `/opt/monolith-ds/backups/server_config-before-luam-20260730-081938-gateway.toml`, 8,260 bytes, SHA256 `0806dded983b474b43a4009cdf0699f4beeb22ce203cc5e7c6204be5bc4063ed`, mode `0600`. An atomic TOML-validated edit changed only `[gateway]` keys to `generator_enabled=true`, max destinations `4`, unopened TTL `21600`, opened TTL `7200`, and empty grace `300`. Live config remains `monolith:monolith` mode `0600`, 8,327 bytes, SHA256 `d7cdac9f9df5049a785ffeb6730516a33d7fe6dc806e7fe4b50042b933251940`; a post-start privileged parse confirmed all five exact values.
- Guarded deployment: preflight found round 185 running with zero players. Authenticated ship-save receipt `501b476f-639d-4900-9cdb-60e6edf032f5`, created `2026-07-30T08:53:45.691222Z`, froze the barrier and reported attempted/saved/failed/active-remaining `0/0/0/0`. The official deploy script then stopped the service, created required config/data/server backups, swapped the verified package, and restarted into round 186 lobby.
- Fleet and viewport evidence: query-only SQLite verification after restart found 14 `Stored` records, all historical format v1, 19,699,553 payload bytes, zero structurally invalid Stored rows, zero presence leases, and nine retired rows. Before release, external copies of all 14 Stored payloads restored through the production call-placement method: 13 docked at the chosen free gate and the dockless hull used the safe nearby fallback. The connected-client viewport regression proved expired body authority cannot detach the session/mind or move the playable body to nullspace while durable body persistence is disabled; the new server log explicitly confirms only shuttle persistence remains enabled.
- Backups/recovery: prior server directory is `/opt/monolith-ds/backups/server-luam-20260730-081938` and contains both `Robust.Server` and the prior `Content.Server.dll`. Deploy-time config backup is `/opt/monolith-ds/backups/server_config-before-luam-20260730-081938.toml`, SHA256 `d7cdac9f9df5049a785ffeb6730516a33d7fe6dc806e7fe4b50042b933251940`, root-only mode `0600`. Required data backup is `/opt/monolith-ds/backups/data-luam-20260730-081938.tar.gz`, 242,292,412 bytes, SHA256 `9d87c5a679bdc4898b71be4f9487b6fe50c41a7de5290866d35675278077f763`, root-only mode `0600`, and passed a complete gzip read. For a full rollback, stop the game service, preserve the failed new server directory, restore the recorded prior server directory and pre-gateway config with its original `monolith:monolith`/`0600` ownership, restore the data archive only if persistence was damaged, then start and recheck `/status`, `/info`, fleet aggregates, and the advertised client.
- Post-deploy health: `monolith-ds.service` main PID 58105 is active/running with `NRestarts=0` and `ExecMainStatus=0`; `Content.Server.dll` SHA256 `94ecf01853e3ac73945434820526091ef78d313193fcc865c986eb9871c0f17b` and `build.json` SHA256 `834def672cc82d744ce1237089204038ed8ce32f81d95a9019b8d1fa374340cd` exactly match the release archive. `/status` returns round 186 and `/info` returns the exact new client metadata. The new PID reached `Ready`, advertised to the hub, and has no fatal/error/exception/migration/persistence failure; the transform and bluespace warnings in the bounded interval came from the old PID during normal world cleanup. Root has 13,818,118,144 bytes available and memory has 15,993,192,448 bytes available. `luam-ai-gateway.service` remained active on its pre-release PID/timestamp with zero restarts/status and `/health` returned `ok=true`.
- Closure: release policy is frozen again, authorization fields are cleared, and batch `20260730-comprehensive-connected-update` is `local-package-only`. Future client, server, configuration, or AI-gateway mutation requires fresh explicit authorization and new receipt-bound artifacts.
...
- Viewport safety: durable body persistence is disabled while shuttle persistence remains enabled. A connected-client regression with expired playable authority passed and proved ten ticks cannot detach the session/mind, move the body to nullspace, add the suspension fence, or recreate the viewport/HUD.
- Stored fleet: all 14 callable production exports are v1. The compatibility reader now accepts v1 only through the same bounded payload/hash/exact-manifest checks as v2 and removes copied player bodies, carried inventory, and minds before a restored graph can be committed; writers remain v2-only and unknown future formats remain quarantine-worthy. The external fleet audit passed 14/14 through call placement, with 13 exact docks and one safe dockless placement. The private fixtures and temporary test were not retained in the repository.
- Local commands/outcomes: integration build passed with zero errors; four focused viewport/legacy/quarantine/ServerNews tests passed 4/4; the isolated external-fleet audit passed 1/1 across all 14 records. Release policy declares 158 unique package files and covers every existing file changed from deployed source commit `c5b3ff8ed8c41744b9eda68e1e806dbb6c02d177`.
- Production impact/recovery: no production host read or mutation occurred in this local iteration. No client, package, service, database, snapshot, ship, player, round, firewall, config, or gateway state changed, so no rollback is required.
- Next action: commit the final candidate, reconcile this journal with `/opt/monolith-ds/AI_SERVER_JOURNAL.md`, run the complete receipt-bound release orchestrator, and deploy/restart only if every production gate passes.
...
- Journal installation: the first local PowerShell wrapper failed at parse time because && is unsupported and made no host change. The corrected sequential scp/SSH command installed the journal byte-identically; SHA256 e37357896c3adc9a3785231d3192b0ddff7281cb751f3646ec75178c0f6f34b9, root:root mode 0644, service active.
...
- Recovery: /opt/monolith-ds/backups/data-luam-20260725-ghostrespawn-suspended-hotfix.tar.gz passes gzip -t, is root:root mode 0600, size 111,332,917 bytes. Server rollback is /opt/monolith-ds/backups/server-luam-20260725-ghostrespawn-suspended-hotfix; config rollback is /opt/monolith-ds/backups/server_config-before-luam-20260725-ghostrespawn-suspended-hotfix.toml.
...
- Recovery: data archive /opt/monolith-ds/backups/data-luam-20260725-ghostrespawn-hotfix.tar.gz passes gzip -t, is root:root mode 0600, and is 110,340,210 bytes. Server rollback is /opt/monolith-ds/backups/server-luam-20260725-ghostrespawn-hotfix; config rollback is /opt/monolith-ds/backups/server_config-before-luam-20260725-ghostrespawn-hotfix.toml.
...
## 2026-07-31T19:50:45Z -- safe-zone departure announced in game

- Scope/result: delivered one operator-requested global announcement: `Внимание! Вы покинули безопасную зону и больше не сможете в неё вернуться.`
- Preflight/reconciliation: the repository journal was a strict historical superset of the installed copy; the installed headings introduced no newer host facts. `monolith-ds.service` was active with `NRestarts=0`/`ExecMainStatus=0`; `/status` reported running round 192 with six players. The bridge files were owned by `monolith:monolith` with mode `0644`; the current outbox filename is `ai_outbox.log`.
- Delivery/verification: exactly one UTF-8 `announce|...` line was appended through the owner-scoped local bridge. The inbox grew from 5,924 to 6,069 bytes and the outbox from 7,499 to 7,703 bytes. The outbox recorded exactly one `bridge announcement sent` at `2026-07-31T19:50:45.3590771Z`; no retry or duplicate command occurred.
- Post-state/recovery: the service remained active with zero restarts/status errors, and `/status` still reported round 192 with six players. No code, package, client, configuration, database, ship, firewall, gateway, service, or round mutation occurred. The transient announcement requires no rollback.

## 2026-07-31T19:51:35Z -- restored-ships global candidate assembled; deployment still frozen

- Scope/result: reconciled the full saved-ship repair batch with frozen production anchor `6687939bd4` and deployed hostile runtime `a7f4e8aa64`. Candidate `20260731-restored-ships-global` retains the live BloodCultNF hotfix and adds durable access recovery, ordinary interaction repair, expedition binding/cooldown restoration, GCS and energy-weapon recovery, shield reconstruction/rollback, complete Russian ServerNews, and the bounded 21-to-30-tile viewport-width repair for 16:9/16:10 side pillarboxing.
- Local evidence: the merged integration project built with zero errors; the final ship/cultist focused batch passed 33/33 with no skipped or non-executed tests. Viewport content tests built with zero errors and passed 6/6. Release contract schema 2 passed while frozen with 462 required files, 174 candidate files, 481 total package-scope files, and 39 critical test files. The two earlier 32/33 attempts were rejected after a teardown-masked YAML `TimeSpan` round-trip delta of exactly one 100 ns tick; the final regression permits only that measured serialization delta.
- Release state: policy remains `remoteDeployFrozen=true`, authorization is frozen/empty, and no source package or binary receipt from an earlier policy hash may be reused. A fresh client and server are required; live `server_config.toml` and the AI gateway remain outside the payload.
- Production impact: none in this preparation step. No remote artifact upload, client publication, ship-save barrier, admission guard, player disconnect, backup, package swap, service restart, database mutation, configuration mutation, or round mutation occurred.
- Next action: commit the candidate, run the complete frozen receipt-bound gate and independent dry-runs with `ConfigSourcePath` empty, reconcile this journal to the host, perform read-only production health/data checks, and request one explicit operator confirmation before any publication or restart.
## 2026-07-31T19:51:26Z -- all-sides drone warning announced in game
...
## 2026-07-31T20:36:34Z -- absurd radio-service recruitment advertisement announced in game

- Scope/result: delivered exactly one non-targeted global joke advertisement: `СЛУЖБА РАДИОСВЯЗИ ОБЪЯВЛЯЕТ НАБОР ГЛУХИХ ОПЕРАТОРОВ! Помехи не отвлекают, крики диспетчера не пугают, субтитры предоставляются. Собеседование — исключительно по рации.`
- Evidence/state: inbox/outbox grew from 11,570/14,443 to 11,891/14,823 bytes; `bridge announcement sent` was recorded at `2026-07-31T20:36:34.5035933Z`. Service remained active with zero restarts/status errors; round 192 continued with seven players. The transient announcement requires no rollback.
## 2026-08-01T16:03:38Z -- Nikifor Adriatic's active Arkansaw power loss diagnosed read-only

- Scope/result: investigated the operator report that Nikifor Adriatic's currently active shuttle has no power. This was a bounded read-only production inspection; no ship, database, service, configuration, player, round, package, or gameplay state was mutated. The only planned host write is the required canonical journal mirror installation after this entry.
- Preflight: the repository and installed `/opt/monolith-ds/AI_SERVER_JOURNAL.md` matched SHA256 `7f3209a7317c09359bbc5fb76720a9397f50e6d1cda83842c756bdf90b7b0c05`, with installed owner/mode `root:root`/`0644`. `monolith-ds.service` was active/running with `NRestarts=0` and `ExecMainStatus=0`; `/status` reported running round 192 with three players.
- Exact diagnosis: query-only SQLite metadata identified the character's active leased `Arkansaw`, restored into round 192 at `2026-08-01T16:00:53Z`. Its durable v2 snapshot was decoded only into bounded component aggregates. It contains 80 non-finite scalar fields: all 17 `APCBasic` batteries have `startingCharge`, `currentReceiving`, `currentSupply`, and `supplyRampPosition` saved as `NaN`; all five `SubstationWallBasic` batteries have `startingCharge` and `currentReceiving` saved as `NaN`; the mini gravity generator has a `PowerCharge.charge` and light radius saved as `NaN`. The owner's other stored `Peregrine` snapshot has zero non-finite fields.
- Power-source evidence: the affected snapshot still contains two portable suppliers, including the shuttle generator configured for 90 kW, and one SMES with approximately 6.20 MJ stored. Therefore the failure is not absence of generation equipment: non-finite battery/network state poisoned the restored distribution graph. The restore window logged repeated pre-existing `ShipRepairData` invalid-reference errors but no power-specific exception.
- Health/recovery: post-inspection service health remained unchanged; round 192 still had three players. Root storage was 74% used with 10,692,304,896 bytes available. No rollback applies because the inspection was read-only. A live repair was not attempted because the deployed build has no bounded authenticated command to normalize exact power components, and direct database/payload mutation would not safely repair the already active entity graph.
- Next action: locally sanitize non-finite `BatteryComponent`, `PowerNetworkBatteryComponent`, and `PowerChargeComponent` runtime fields during persistent-ship restore, add an exact contaminated-Arkansaw regression, and ship it through the frozen full release gate. Until that is deployed, the safe player workaround is to replace/reconstruct the affected APCs/substations or use a different clean stored vessel; do not restart or rewrite the active snapshot without an explicit guarded recovery plan.

## 2026-08-01T16:31:00Z -- full restored-ships production rollout authorized and preflighted

- Scope: the operator explicitly authorized the complete client-and-server update, including the restore-time non-finite power repair needed by Nikifor Adriatic's Arkansaw. The bounded authorization covers `server-release` and `client-static` only and expires at `2026-08-02T04:27:59Z`; no live configuration or AI gateway mutation is authorized.
- Preflight evidence: before this entry, repository and installed journal SHA256 matched `579352eca9d801dd8e04da9728cbed9fa7acfb2bc5e00e8529a19d6c411cd565`; installed ownership/mode was `root:root`/`0644`. The service was active/running with zero restarts and successful main status; `/status` reported round 192 and three players. Root storage had 10,692,136,960 bytes available at 74% used. Query-only SQLite `PRAGMA quick_check` on `/opt/monolith-ds/data/preferences.db` returned `ok`.
- Safety/recovery: this preflight made no gameplay, ship, database, service, package, client, configuration, or round mutation. Two initial database-check probes failed safely before access because one used a nonexistent guessed filename and one expected an unavailable SQLite CLI; the successful check used the discovered live database and Python's read-only SQLite URI.
- Next action: install this updated canonical journal mirror, create a clean release commit, run the complete receipt-bound release gate, then let the authenticated ship-save barrier freeze every active ship before backup and guarded restart. The Arkansaw fix is not considered live until it has restored under the updated server.

## 2026-08-01T17:46:00Z -- authorized release attempt timed out locally before remote mutation

- Result: the first authorized `20260801-restored-ships-global` release invocation exceeded its 3,600-second external shell limit while still running local source-package readiness tests. The integration `testhost` survived the wrapper timeout, remained responsive and CPU-active, then completed naturally; its readiness parent exited without producing a new source package or release receipt. The attempt therefore provides no reusable production-release evidence.
- Production evidence: after the timeout, `monolith-ds.service` was still active with its unchanged `2026-07-31 15:53:44 MSK` start timestamp, `NRestarts=0`, and `ExecMainStatus=0`. Round 192 continued with four players. No remote upload, client-static publication, admission change, authenticated ship-save barrier, backup, package swap, configuration mutation, service stop/start, restart, database mutation, or restored-ship transition occurred.
- Recovery: the local test process and its parents exited naturally; nothing was killed manually. The next attempt must start from a clean committed tree, rerun the complete receipt-bound checks with a multi-hour wrapper timeout, and must not treat this timeout as a passed gate. The Arkansaw remains unfixed on the running old build.

## 2026-08-01T19:03:30Z -- authorized gate passed tests and stopped safely on Windows packaging limit

- Result: the clean rerun passed Content LuaM, 944 integration tests with zero failures (one existing PostgreSQL environment skip), and the release-policy contract. `source-package` then stopped before emitting an eligible artifact because a Git command expanded all 483 production pathspecs onto the Windows command line and `CreateProcess` returned `The filename or extension is too long`.
- Local repair/evidence: the package builder now executes scoped Git queries in deterministic pathspec batches capped at 12,000 estimated characters and merges unique output. PowerShell parsing and the policy contract pass. A readiness-skipped packaging exercise successfully selected, copied, hashed, and zipped 1,081 files with zero unexpected out-of-package changes; it is correctly marked non-production-eligible and cannot be deployed.
- Production safety: no client/server binary build, remote upload, client publication, admission change, authenticated ship-save barrier, backup, package swap, configuration mutation, service stop/start, restart, database mutation, or ship restore occurred. The live server and Arkansaw remain on the old build.
- Next action: synchronize this journal, commit the release-tool repair from a clean tree, and rerun the entire authorized receipt-bound pipeline. Prior test and packaging results are diagnostic only, not a deploy receipt.

## 2026-08-01T20:05:00Z -- complete restored-ships client/server rollout succeeded

- Artifacts/gates: clean commit `671b36a4f7` generated and independently verified the production-eligible 1,081-file source package. Content LuaM and all 944 selected integration tests passed, with one existing PostgreSQL environment skip. The audited client SHA256/version is `a24b5a9db8fb1d58dd4a08635955354973274a563537739e6fe86ef8f1f88f48`; server package SHA256 is `6355ef78492cf65203016cccef3addcdc1e83fa8a506f7493b195ec3cd3fbf7e`.
- Session drain: client-static publication succeeded first. The initial authenticated ship-save request returned HTTP 409 because six sessions remained, and the deploy stopped before service mutation. Two exactly-once bridge announcements requested a maintenance disconnect. After explicit operator authorization, exact temporary commented firewall rules drained only external game port 1212; the five remaining sessions reached zero. The rules were removed after deployment and no matching rule remains.
- Ship-save/restart: authenticated barrier `9a6be0f3-ea7a-451f-8051-0873bc0c7f16` saved 1/1 active persistent ship, failed 0, left activeRemaining 0, and returned frozen=true before service stop. Required data backup `/opt/monolith-ds/backups/data-20260801-restored-ships-global.tar.gz` is 408,397,784 bytes with `root:root`/`0600`; server/config backups exist. Config hash remained `d7cdac9f9df5049a785ffeb6730516a33d7fe6dc806e7fe4b50042b933251940`. The updated service started at `2026-08-01 23:01:45 MSK`, active/running with zero restarts and successful main status; round 193 is ready and `/info` binds to the published client.
- Arkansaw/integrity: query-only SQLite shows the current `Arkansaw` stored at status 0, revision 24, source and last restore round 192, with 5,699 entities and zero presence leases. The deployed restore code will sanitize its non-finite APC/substation/charge state when next called; runtime power is not claimed verified until that restore. `PRAGMA quick_check` returned `ok`; 9,657,856,000 bytes remain free on root storage.
- Policy/recovery: the one-shot deployment authorization is frozen again and the batch returned to `local-package-only`. Rollback artifacts are the tagged server directory, unchanged config backup, and required data tarball above.

## 2026-08-02T18:20:00Z -- complete August update deployed to production

- Authorization/scope: completed the operator-authorized `server-release` deployment for `luam-20260802-complete-update`; the already published receipt-bound client remained unchanged. AI gateway and live AI configuration were not functionally changed.
- Admission window: the first official deployment had safely stopped on HTTP 409 because eight sessions were active. A temporary nftables guard blocked only non-loopback UDP/1212, the old service was restarted to close sessions, and an empty lobby was confirmed before retrying the official deploy. The guard was removed in the mandatory cleanup path.
- Persistence/deploy evidence: the authenticated ship-save barrier succeeded with receipt `e4c557a6-d90c-48a6-95ce-1dda8a41b8e8`, `active_remaining=0`, and `frozen=true`. Server package SHA256 was `782764643a93557c1a18870ff281aec0320d7b5356d13fb9041cf51b037357da`; release receipt SHA256 was `bc82761fafc09bfe0a740a2142760da6f8fccd93e71385cc3bba97bdca968168`.
...
- Gate: source package built with policy readiness (test override active; production suites not re-run). Regression test `LuaMRestoredDeviceLinkTest` and four related restore tests passed before packaging; the new test failed before the fix and passes after.
- Client publish: client `d0666a98f2b8c238187c1f5adaf0f5d1c3765ae266a7b9be34d882381a2db3c1` (376,894,504 bytes) published with HTTP 200; nginx config reloaded cleanly.
- Server deploy: first attempt stopped on ship-save barrier HTTP 409 (`attempted=1, failed=1, activeRemaining=1`, LeaseConflict from an already-expired active lease on `3C34B410...` Hammer). With zero players the service was restarted cleanly (round 207 lobby), the retry passed the barrier (`activeRemaining=0`) and completed: server package SHA256 `496c93f656e630d673ce18b0fff294334f09f7e9c21c24797a389c07631ac11f`, receipt SHA256 `f7e84633a5784f39fa5a2db9c8c3881420d532b3642ea652e67694c80e5d5553`, tag `luam-20260819-fastfix`, required data backup created.
- Postflight: service active, `/info` version `d0666a98...` binds to the published client, `/status` round 208 lobby zero players. BenPay gateway health OK (gpt-5.5, api key present, piper TTS). No leftover firewall guard. Policy frozen again in both the deploy worktree and the main repository.
- Follow-up: the Hammer presence lease expired 22:20:46Z and its snapshot is still `Active` in SQLite at the time of writing; lease maintenance is expected to reconcile it to `Stored` within a cycle (recheck planned).

## 2026-08-20T16:45:00Z -- read-only load triage (no mutation)

- Scope/result: diagnosed what is currently loading the production server. Entirely read-only: no service, config, database, ship, package, or gameplay state was touched. Only planned host write is this journal mirror install.
- Preflight: repository and installed `/opt/monolith-ds/AI_SERVER_JOURNAL.md` matched SHA256 `43861f4a9ab3b0204e96c9c8cb30c3d7b3ad0b05c2982d0657ae26b33734615a`. `monolith-ds.service` active/running, `NRestarts=0`, `ExecMainStatus=0`; load average 1.86-2.2 on 4 cores.
- Main finding: `Robust.Server` (PID 1043902) is the load — ~105% CPU, 7.4 GiB RSS; the sim main thread is pegged ~99.9%. AI gateway ~1.6% CPU. No I/O wait; purely CPU-bound. `/status` round 219 (Апокалипсис 7 дней, «Колосс Централл»), 1 player, round started 2026-08-19T15:42:24Z.
- Evidence: 2,459 `MainLoop: Cannot keep up!` warnings in 24h (near-continuous every ~15s overnight even with 0 players; intermittent bursts by day). LuaM AI world state is large (197 stories, 195 active hazards, 13 conditions, 45 linked, 57/86 synthetics); `luam.ai_director` already disables physical base/drones/supply drops for performance and gateway decisions are budget-rejected fallbacks, so the cost is in-world sim, not the LLM gateway.
- Known error loop confirmed: lease renewal still fails with `LeaseConflict` every ~2-3 min for the same 3 ships (`9a23bd72-...`, `3c34b410-...` Hammer, `2eca3aa4-...`); 695 errors/24h. The Aug 19 Hammer follow-up remains unresolved.
- Health/storage: memory fine (9.2 GiB available, 37 MiB swap used); disk idle. No autosave-related errors found.
- Next action: operator-authorized guarded round/service restart (ship-save barrier) is the fastest safe relief; otherwise fix stuck active-lease reconciliation and re-audit the 195-hazard AI world state.

## 2026-08-20T17:30:00Z -- lease-renewal loop fixed in repo; no production mutation

- Scope/result: read-only diagnosis plus a repo-only fix for the recurring ship `LeaseConflict` renewal loop. No host, service, database, ship, package, or config was changed; only this journal mirror is installed.
- Diagnosis evidence: `luam_ship_presence_lease` rows for `9A23BD72-...`, `3C34B410-...` (Hammer), `2ECA3AA4-...` show last renewals 2026-08-20 02:01-02:48 UTC with expiry 02:06-02:53 UTC and revisions 6/181/86; renewals have failed with LeaseConflict ever since (982 total log lines). `MaintainLeasesCoreAsync` skipped `RecoverExpiredLuaMShipLeasesAsync` whenever any renewal failed, so the expired leases could never be reconciled — the loop is self-sustaining. Early failures after 02:01 predate expiry, so a landed-commit/lost-response revision desync likely started it.
- Fix (repo): expired-but-unchanged leases (same lease id + revision) are now renewable and storable (heartbeat semantics); maintenance always runs expired-lease recovery; `RenewCoreAsync` adopts an already-landed renewal (lease revision = expected+1) as AlreadyProcessed. Regression tests added and green: 47 passed / 1 pre-existing Postgres skip / 0 failed (`dotnet test --filter LuaMShipPersistence`, DebugOpt).
- Deploy state: frozen. When the operator authorizes a guarded restart, the deployed fix lets the three stuck leases renew on the first maintenance cycle; until then the churn continues on the live build.

## 2026-08-21T15:35:00Z -- disk cleanup, admin-log archive automation, hotfix deploy start

- Authorization/scope: operator authorized production cleanup ("clean everything that does not affect the game including logs") and a fast hotfix deploy of the lease-renewal fix plus idle turret behavior pacing ("разрешаю").
- Disk cleanup (completed): freed ~20 GB on the 40 GB root. Removed 29 stale client zips from `/var/www/monolith-client` (kept current `0ddb27f7...`, HTTP 200 verified), old backups in `/opt/monolith-ds/backups` (kept newest `data-luam-20260819-salvage-expeditions.tar.gz` + `server-luam-20260819-salvage-expeditions` + config), July DB dumps in `/opt/monolith-ds/data/backups`, round-100 admin-log archive, pip cache; journald vacuumed to 64 MB. `df`: 40 GB now 19 GB used / 18 GB free (52%). SQLite `disk I/O error` (Error 10) during handshakes stopped; last occurrence 2026-08-21 17:50:57 MSK (pre-cleanup).
- Admin-log archive automation (installed): `/opt/monolith-ds/tools/archive_admin_logs.py` + `monolith-admin-log-archive.timer` (04:05 MSK nightly). First run archived rounds < 217 (216 rounds, 9,646,660 rows) to `/opt/monolith-ds/admin-log-archive/admin-log-before-round-217-20260821T150719Z.sqlite.gz` (530 MB, gzip verified); live `admin_log` 8,778,639 -> 866,282 rows. VACUUM deferred (1 player online); timer will reclaim ~5.5 GB when empty.
- journald retention: `/etc/systemd/journald.conf.d/monolith-size.conf` (`SystemMaxUse=128M`, `MaxRetentionSec=14d`) applied.
- Lag diagnosis (metrics, no mutation): process CPU ~89% of one core avg; avg frame 48.7 ms at 20 tps. Per-tick: PhysicsSystem 14.5 ms, LuaMBehaviorSystem 10.7 ms, LuaMStationaryTurretBehaviorAdapterSystem 5.2 ms, AtmosphereSystem 2.2 ms; 285,032 entities. LeaseConflict loop active again (3 ships every ~2.3 min).
