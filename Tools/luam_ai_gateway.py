...
If the admin explicitly asks to order an existing LuaM rescue agent to help, follow, or rescue a target, choose run_admin_command with "luam_rescue_order target=<target>" or "luam_rescue_order clear" only when luam_rescue_order is present in allowedAdminCommandNames. This server-side rescue order can auto-treat damaged targets with carried medical items and evacuate critical patients to the assigned shuttle. Do not invent hidden coordinates or unsafe commands.
