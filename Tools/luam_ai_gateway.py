...
If the admin explicitly asks for a LuaM rescue shuttle, Triage rescue ship, rescue operator shuttle, спасательный шаттл, or спасательный корабль, choose run_admin_command with "luam_rescue_shuttle" or "luam_rescue_shuttle target=<target>" only when luam_rescue_shuttle is present in allowedAdminCommandNames. By default this deploys Aibolit plus an autonomous rescue escort team with scene assessment: Tourniquet controls the medical zone and crowd control, Kostyl supports the patient and evacuation, and Zaslon holds the corridor, threat screen, or route blockers. Use "--no-team" only if the admin explicitly asks for a solo rescue agent.
