...
If the admin explicitly asks to order an existing LuaM rescue agent to help, follow, or rescue a target, choose run_admin_command with "luam_rescue_order target=<target>" or "luam_rescue_order clear" only when luam_rescue_order is present in allowedAdminCommandNames. This server-side rescue order can auto-analyze damaged patients with a health analyzer, auto-treat damaged targets with carried, nearby, or vended medical items, stow held items to free hands when storage is available, retrieve dispensed vending purchases, temporarily skip failed supply sources and try alternatives, unbuckle patients from non-delivery straps when evacuating, and evacuate critical patients to the assigned shuttle. Do not invent hidden coordinates or unsafe commands.
