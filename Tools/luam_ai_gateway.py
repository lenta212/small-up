...
If the admin explicitly asks to order an existing LuaM rescue agent to help, follow, or rescue a target, choose run_admin_command with "luam_rescue_order target=<target>" or "luam_rescue_order clear" only when luam_rescue_order is present in allowedAdminCommandNames. This server-side rescue order can auto-analyze damaged patients with a health analyzer, auto-treat damaged targets with carried, nearby, or vended medical items, stow held items to free hands when storage is available, and evacuate critical patients to the assigned shuttle. Do not invent hidden coordinates or unsafe commands.
If the admin explicitly asks an existing LuaM rescue agent to interact, alt-interact, use a held item, treat/heal/analyze a target using a medical item, vend/buy/dispense a medical item from a targeted vending machine, pick up an item, equip the active hand item into an inventory slot, unequip an inventory slot into a free hand, store the active hand item into storage worn in a slot, take an item from storage worn in a slot, pull/drag a target, stop pulling, buckle the currently pulled entity to a strap/bed, or drop an item, choose run_admin_command with "luam_rescue_action action=<interact|alt|use|treat|vend|pickup|drop|pull|stop-pull|buckle> target=<target>", "luam_rescue_action action=treat target=<target> item=<name|prototype|entity> slot=<slot>", "luam_rescue_action action=vend target=<vendingMachine> item=<name|prototype>", a targetless "luam_rescue_action action=<drop|stop-pull>", "luam_rescue_action action=<equip-slot|unequip-slot|store-slot|take-storage> slot=<slot>", or "luam_rescue_action action=take-storage slot=<slot> item=<name|prototype|entity>" only when luam_rescue_action is present in allowedAdminCommandNames. Do not invent targets. Common slots are belt, back, suitstorage, outerClothing, pocket1, pocket2, jumpsuit, id, mask, gloves, head, eyes, ears, and neck. Common medical items are medipen, hypospray, gauze, ointment, brutepack, and analyzer.
