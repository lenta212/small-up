...
public sealed class AdvDrainSystem : SharedDrainSystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var drain))
        {
            ...
            if (!TryComp(uid, out TransformComponent? xform) || !xform.Anchored)
            {
                ...
                SetVisualState(uid, false, false, false);
                continue;
            }

            // not powered
            var energyRequired = drain.Wattage * drain.DrainFrequency;
            if (!_powerCell.HasCharge(uid, energyRequired))
            {
                _ambientSoundSystem.SetAmbience(uid, false);
                SetVisualState(uid, false, false, false);
                continue;
            ...
            SetVisualState(uid, true, false, false);
            ...
            // Remove a bit from the buffer before checking capacity. If the solution is completely full,
            // this is what makes room for the drain to continue operating instead of getting stuck forever.
            if (drainSolution.Volume > drain.UnitsDestroyedThreshold)
            {
                var excess = (float) drainSolution.Volume - drain.UnitsDestroyedThreshold;
                var amountToDestroy = Math.Min(drain.UnitsDestroyedPerSecond * drain.DrainFrequency, excess);
                if (amountToDestroy > 0f)
                {
                    _solutionContainerSystem.SplitSolution(drain.Solution.Value, amountToDestroy);
                    _appearanceSystem.SetData(uid, AdvDrainVisualState.IsVoiding, true);
                    // These use the same indicator light and cause artifacts when on at the same time.
                    _appearanceSystem.SetData(uid, AdvDrainVisualState.IsRunning, false);
                }
            }

            // Manual-only drains still process their buffer/overflow tray.
            if (!drain.AutoDrain)
            {
                _ambientSoundSystem.SetAmbience(uid, false);
            ...
            _puddles.RemoveWhere(puddle =>
                TerminatingOrDeleted(puddle.Owner) ||
                !_interactionSystem.InRangeUnobstructed(uid, puddle.Owner, drain.Range));
            ...
            if (!_powerCell.TryUseCharge(uid, energyRequired))
            {
                _ambientSoundSystem.SetAmbience(uid, false);
                SetVisualState(uid, false, false, false);
                continue;
    ...
    private void SetVisualState(EntityUid uid, bool isRunning, bool isDraining, bool isVoiding)
    {
        _appearanceSystem.SetData(uid, AdvDrainVisualState.IsRunning, isRunning);
        _appearanceSystem.SetData(uid, AdvDrainVisualState.IsDraining, isDraining);
        _appearanceSystem.SetData(uid, AdvDrainVisualState.IsVoiding, isVoiding);
    }
