...
public sealed partial class CryoSleepSystem
{
    ...
    {
        if (!_storedBodies.TryGetValue(id, out var storedBody))
            return;

        _storedBodies.Remove(id);
        DeleteBodyIfStillStored(storedBody.Body);
    }

    /// <summary>
    /// Expires one exact storage episode. A timer from an older stay must not
    /// remove or delete a newer episode for the same user or body.
    /// </summary>
    internal bool ResetCryosleepState(NetUserId id, EntityUid expectedBody, Guid expectedEpisodeId)
    {
        if (!_storedBodies.TryGetValue(id, out var storedBody) ||
            storedBody.Body != expectedBody ||
            storedBody.EpisodeId != expectedEpisodeId)
        {
            return false;
        }
    ...
    private void DeleteBodyIfStillStored(EntityUid body)
    {
        if (Exists(body) && _storageMap is { } storageMap && Transform(body).ParentUid == storageMap)
            QueueDel(body);
    }
