...
[TestFixture]
[NonParallelizable]
public sealed class LuaMCargoShuttleConsoleBindingTest
{
    [Test]
    public async Task CargoShuttleConsoleOnOrdinaryShuttleCreatesLocalOrderDatabase()
    {
        ...
        try
        {
            ...
            await server.WaitAssertion(() =>
            {
                Assert.That(entities.TryGetComponent<StationCargoOrderDatabaseComponent>(station, out var database), Is.True,
                    "Installing a cargo shuttle console on a shuttle station should bind it to that shuttle's own cargo order database.");
