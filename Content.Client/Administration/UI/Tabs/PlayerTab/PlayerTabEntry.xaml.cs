...
public sealed partial class PlayerTabEntry : PanelContainer
{
        ...
        AdminPlayerTabSymbolOption symbolSetting)
    {
        ...
        CameraButton.Disabled = !player.Connected || player.NetEntity == null;
        CameraButton.OnPressed += _ =>
        {
            if (player is not { Connected: true, NetEntity: { } target })
                return;

            _console.ExecuteCommand($"camera {target}");
        };
    }
}
